class pokemonEv():
    def singlesStatsFunct(stat_type,basestat,lvl,iv,ev,stat,nature):
        if stat_type == 'hp':
    ...
    def allStatsFunct(lvl,base,ivcal,evcal,add_stat,nature):
        ev_needed = [0,0,0,0,0,0]
        for x in range(len(ev_needed)):
            d = (2 * base[x] + ivcal[x] + (evcal[x]/4)) * (lvl/100)
            ev_needed[x] = (((((add_stat[x]/nature[x])-d)*400)/100)/4) + 4
            x = x + 1
